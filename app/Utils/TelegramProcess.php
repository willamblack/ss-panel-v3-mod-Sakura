...
class TelegramProcess
{
    private static function needbind_method($bot, $message, $command, $user, $reply_to = null)
    {
        ...
        ", $parseMode = null, $disablePreview = false, $replyToMessageId = $reply_to);
                ...
                case 'checkin':
                    ...
                    break;
                case 'lottery':
                    $price = explode(" ",$message->getText())[1];
                    $price = number_format($price,2,".","");    //format price number
                    if ($price == 0.00){
                        $bot->sendMessage($message->getChat()->getId(), "乃真的输入了需要花费的金钱了吗？请在 \"/lottery\" 后加上半角空格后再输入金额。", $parseMode = null, $disablePreview = false, $replyToMessageId = $reply_to);
                        break;
                    }
                    if ($user->money < $price){
                    ...
                    $user->money -= $price;
                    $user->save();
                    $bot->sendMessage($message->getChat()->getId(), "乃通过花费 ".$price." CNY 余额抽奖获得了 ".$traffic." MB 的流量~~", $parseMode = null, $disablePreview = false, $replyToMessageId = $reply_to);
                    break;
                default:
    ...
    public static function process()
    {
        try {
            ...
            $command_list = array("ping", "chat" ,"traffic", "help", "checkin", "lottery");
